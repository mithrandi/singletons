  ...
  withTestLBI pkg lbi $ \suite suitecfg -> when (testName suite == fromString testSuiteName) $ do
    ...
    withLibLBI pkg lbi $ \_ libCLBI -> do
      let libDeps = map fst $ componentPackageDeps libCLBI
          ...
          singletonsBatteriesUnitId = componentUnitId libCLBI
          deps = formatDeps (singletonsBatteriesUnitId:libTransDeps)
          allFlags = dbFlags ++ deps
      writeFile buildSingletonsFile $ unlines
        [ "module Build_singletons_batteries where"
        , ""
        , "ghcPath :: FilePath"
        , "ghcPath = " ++ show ghc
        , ""
...
buildSingletonsModule = "Build_singletons_batteries"
