...
import Build_singletons_batteries  ( ghcPath, ghcFlags, rootDir )
...
normalizeOutput file = Turtle.inplace pat (fromString file)
  where
    ...
    pat = asum
      ...
      -- Remove pretty-printed references to the singletons-batteries package
      -- (e.g., turn `singletons-batteries-2.4.1:Sing` into `Sing`) to make the output
      -- more stable.
      , "" <$ "singletons-batteries-" <* verNum <* ":"
      ]
